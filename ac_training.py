...
parser.add_argument('--buffer_length',  type=int, default=5000, help='Maximum number of memories stored in the memory buffer before overwriting. ')
parser.add_argument('--timestep_to_start_learning',  type=int, default=200, help='Timestep at which we start updating the agent. ')
...
policy_net = create_sequential_model(num_inputs=env.observation_space.shape[0], layers_spec=layers_spec, num_outputs=env.action_space.n,
                                  dropout_rate=0, activation_function='relu', final_activation=False)
value_net = create_sequential_model(num_inputs=env.observation_space.shape[0], layers_spec=layers_spec, num_outputs=1,
                                     dropout_rate=0, activation_function='relu', final_activation=False)
